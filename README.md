...
1. **首次绑定后弹窗询问**：可在页面上选择是否立即拉取最近 60 天历史数据，选择「暂不」不影响后续使用。
2. **每日定时同步**：每天 **09:00（Asia/Shanghai）** 自动为所有已连接且开启自动同步的账号拉取数据，默认回溯 3 天。
3. **手动触发**：账号列表的「同步」按钮拉取最近 7 天；接口可通过 `days` 指定回溯天数。

实测耗时：3 天约 5 秒，30 天约 40 秒，90 天约 90 秒（逐日拉取）。

定时任务参数：`app.sync.daily-cron`（默认 `0 0 9 * * *`）、`app.sync.daily-days`（默认 3）、`app.sync.zone`（默认 `Asia/Shanghai`）。已有未完成任务在飞的账号会被跳过，避免同一账号并发同步。
