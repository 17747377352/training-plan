...
**两个步骤要两个配对码**：配对码是一次性的（Redis 取用即删），令牌那一步用的是
`POST /api/garmin/accounts/pair`，本机浏览器这一步用的是 `POST /api/garmin/browser-upload/pair`，
两条路消费的是**同一批码**。所以第 1~2 步用掉一个码之后，第 3 步要回页面**再领一个新的**；
复用同一个会得到 `46011 配对码无效或已过期`。
...
VO2max 与体能年龄端点穿过请求及落盘白名单（防止只读到旧表而日常不刷新）、
...
上传重试策略（5xx/429 重试到成功、4xx 立刻停手、次数用尽后如实报错）、
